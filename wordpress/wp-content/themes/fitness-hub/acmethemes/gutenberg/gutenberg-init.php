...
function fitness_hub_dynamic_editor_styles(){
	...
	$fitness_hub_link_color               = esc_attr( $fitness_hub_customizer_all_values['fitness_hub-link-color'] );
	$fitness_hub_link_hover_color         = esc_attr( $fitness_hub_customizer_all_values['fitness_hub-link-hover-color'] );
...
function fitness_hub_block_editor_styles() {
	wp_enqueue_style( 'fitness_hub-googleapis', '//fonts.googleapis.com/css?family=Open+Sans:300,300i,400,400i,600,600i,700,700i,800,800i', array(), null );
	wp_enqueue_style( 'fitness_hub-block-editor-styles', get_template_directory_uri() . '/acmethemes/gutenberg/gutenberg-edit.css', false, '1.0' );

	/**
	 * Styles from the customizer
	 */
	wp_add_inline_style( 'fitness_hub-block-editor-styles', fitness_hub_dynamic_editor_styles() );
}
add_action( 'enqueue_block_editor_assets', 'fitness_hub_block_editor_styles',99 );

function fitness_hub_gutenberg_scripts() {
	wp_enqueue_style( 'fitness_hub-block-front-styles', get_template_directory_uri() . '/acmethemes/gutenberg/gutenberg-front.css', false, '1.0' );
	wp_style_add_data( 'fitness_hub-block-front-styles', 'rtl', 'replace' );
}
